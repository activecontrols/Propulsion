...
%% CALCULATIONS
% Tank Calcs
tank_radius = (tank_OD - 2 * wall_thickness) / 2; % LOx tank inner radius (in)
...
h_lox = (lox_vol * 12^3) / (pi * tank_radius^2);

% Tank Mass
if (tank_radius * 2) > h_lox || (tank_radius * 2) > h_ipa
    fprintf("\n\nTANK IS SHORTER THAN IT IS WIDE\n")
    output = 0;
else
    output = 1;

    bulkhead_vol = 4/3 * pi * (tank_radius^3 - (tank_radius - wall_thickness)^3); % Tank material volume of bulkheads (in^3)
    clyinder_vol_ipa = pi * h_ipa * (tank_radius^2 - (tank_radius - wall_thickness)^2); % IPA tank material volume of cylinder section (in^3)
    clyinder_vol_lox = pi * h_lox * (tank_radius^2 - (tank_radius - wall_thickness)^2); % LOx tank material volume of cylinder section (in^3)
