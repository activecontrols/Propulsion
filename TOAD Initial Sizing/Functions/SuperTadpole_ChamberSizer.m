...
Vcon = 1/3 * pi * (Dc/2 - Dt/2)*tand(Convergence_angle) * ((Dt/2)^2 + (Dt/2)*(Dc/2) + (Dc/2)^2); % Converging Section Volume (in^3)
Vc = L_star * At; % Total Chamber Volume (in^3)
Ac = pi * Dc^2 / 4; % Chamber Area (in^2)
Lc = (Vc - Vcon) / Ac; % Chamber Length (in)
