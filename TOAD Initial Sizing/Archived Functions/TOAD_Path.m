function [propellantMassFinal, cutoffTime] = TOAD_Path(mdot)
    %Stathis Kokkevis
    ...
    TWR = 1.1:0.1:1.4; %Thrust to weight ratio of lander
    
    
    cutoffTime = -1;
    
    breakTime = 5:1:15; % Seconds splitting up ascent and descent
    
    
    totalTime = 0:0.1:20;
    
    
    heightToad = zeros(1, length(totalTime)); % Height of TOAD at every 0.1 sec in the mission
    massToad = zeros(1, length(totalTime)); %Mass of TOAD at every 0.1 seconds in mission
    velocityToad = zeros(1, length(totalTime));
    accelerationToad = zeros(1, length(totalTime));
    idealThrottleProfile = zeros(1, length(totalTime)); %Ideal Throttle Profile for ascent
    ...
    for k = 1:length(breakTime)
        timeAscent = 0:0.1:breakTime(k); % Time interval for the ascent phase (seconds)
        timeDescent = breakTime(k)+0.1:0.1:20; % Time interval for the descent phase (seconds)
        
    
        for l = 1:length(TWR)
    
            fullyLoadedMass = thrust(end) / TWR(l); %Fully loaded mass of lander, in lbs
            ...
            for i = 1:length(throttleDown)
                for j = 2:(length(timeAscent) - 1)
                    ...
                    ascentMassDown = ascentMassFull(end) - mdot(i) .* ascentTimeIntervalDown;
            ...
            for i = 1:length(throttleUp)
                for j = 2:(length(timeDescent) - 1)
                    ...
                    % (throttled down)
                    descentMassDown = endAscentMass - mdot(1) .* (descentTimeIntervalDown - breakTime(k));
                    ...
                    descentMassFull = descentMassDown(1) - mdot(i) .* (descentTimeIntervalFull - breakTime(k));
