function [MaxAlt, rail_speed, Mach_num_max, MaxAccel] = ...
    Traj_1DoF_Model_Throttling_V2(m_dry, Cd, outerDiameter, thrust_lbf, t_b, isp_ex, printResults, minThrottle)
...
Inputs:
  ...
  7. Print Results: 'printResults' [boolean]
  8. minThrottle: minimum throttle percentage [0-1]
Outputs:
...
% [MaxAlt, AltMaxV, Mach_num_max, MaxAccel] = Traj_1DoF_Model_Throttling_V2(100, .4, 6.5, 900, 10.3, 200, 1, 1)
...
m_prop = impulse / (isp_ex * g_0); % kg

% Calculate mass flow rate
m_dot = m_prop / t_b; % kg/s

% Throttling Values
sum = 0;
n=10000;
for i = 1:n
    sum = sum + thrustCurve1(i,n,minThrottle);
end
avgThrottle = sum/n;
...
[atmosData(:,2), atmosData(:,1)] = stdatmo(0:altStep:atmosAltMax);
...
while v >= 0
    ...
    if t < t_b
        thrustInst = thrust * thrustCurve1(t, t_b, minThrottle);
        thrustInstMat(index) = thrustInst;

        m_dot = thrustInst / (isp_ex * g_0);
