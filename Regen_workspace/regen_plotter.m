...
%subplot(2,1,2)
yyaxis left
heatflux_plot = plot(x_plot .* 1000, qdot_g ./ 1000, 'red', 'LineStyle', '-',"LineWidth",2);
ylabel('Heat Flux [kW/m^2]')
set(gca, 'Ycolor', 'k')
ylim([0 20000])
heatflux_area = area(x_plot .* 1000, qdot_g ./ 1000);
heatflux_area.FaceColor = "#FFA9A9";
heatflux_area.FaceAlpha = .5;
heatflux_area.EdgeAlpha = 0;
bulk_heatflux = trapz(x_plot.*1000, qdot_g./1000);
fprintf("Bulk Heatflux (kW/mk): %.02f\n", bulk_heatflux)

grid on

yyaxis right
chamber_contour = plot(x_plot .* 1000, r_interpolated .* 1000.*0.03937008, 'black', 'LineStyle', '-',"DisplayName","chamber contour");
ylabel('Radius [mm]')
set(gca, 'Ycolor', 'k')
ylim([.5 3.5])
axis equal;

legend([heatflux_plot, chamber_contour], 'Convective Heat Flux', 'Chamber Contour','Location','best')
...
%subplot(2,1,2)
yyaxis left
heatflux_plot = plot(x_plot .* 1000.*0.03937008, qdot_g ./ 1000 .*0.000611902914742, 'red', 'LineStyle', '-',"LineWidth",2);
ylabel('Heat Flux [Btu/s/in^2]')
set(gca, 'Ycolor', 'k')
ylim([0 14])
heatflux_area = area(x_plot .* 1000.*0.03937008, qdot_g ./ 1000.*0.000611902914742);
heatflux_area.FaceColor = "#FFA9A9";
heatflux_area.FaceAlpha = .5;
heatflux_area.EdgeAlpha = 0;
bulk_heatflux = trapz(x_plot, qdot_g./1000).*trapz(x_plot,w_c_x);
bulk_heatflux_btu = bulk_heatflux * 0.000611902914742;
fprintf("Bulk Heatflux [Btu/s/in^2]: %.02f\n", bulk_heatflux_btu)

grid on

yyaxis right
chamber_contour = plot(x_plot .* 1000.*0.03937008, r_interpolated .* 1000.*0.03937008, 'black', 'LineStyle', '-',"DisplayName","chamber contour");
ylabel('Radius [inches]')
set(gca, 'Ycolor', 'k')
ylim([.5 3.5])
axis equal;
