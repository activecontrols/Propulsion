function [data] = cea_rocket_read(file_name, data, verbose)
...
%   verbose: Prints warnings if true (1); otherwise only prints errors
%
% Output
...
%   Switch containers.Map() output to structs.
...
while ischar(line)
    ...
    elseif startsWith2(line, 'O/F=')
        ...
        for i = 1:2:7
            ...
            % NASA CEA outputs ******* if phi is over 100 instead of the
            % actual value. Ignore phi in this case. TODO: Calculate phi
            % based on O/F??? Or find a CEA input that fixes problem.
            if startsWith(ratios_split{i+1}, '*')
                ratios_split{i+1} = '-1';
            end
    ...
    elseif startsWith2(line, 'CHAMBER') || startsWith2(line, 'INJECTOR')
        ...
        while (strcmp(map('type'), 'fr') ...
            ...
            % Make products the correct case as found in thermo.inp.
            if startsWith2(line, 'MOLE')
                is_lower = false;
            end
            ...
            key = cea_rocket_get_key_name(key, is_lower);
...
%             vals = strsplit(strtrim(line(key_cutoff:end)));
            ...
            if strcmp(key, 'p')
                conversion = 1e5;
            elseif strmatch(key, {'h', 'u', 'g', 's', 'cp', 'cp_fr'}, 'exact')
                conversion = 1e3;
...
%         map_keys = keys(map);
    ...
    elseif startsWith2(line, 'THAN 50 K')
        ...
        if verbose
            fprintf(strcat('WARNING: Condensed species at p = %f Pa, ', ...
                'o/f = %f\n'), map('pin'), map('o/f'));
        end
    end
